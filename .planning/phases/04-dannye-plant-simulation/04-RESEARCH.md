...
Phase 4 — финальная фаза milestone v1. Инфраструктура полностью готова: Rust запускает PlantSim через PowerShell `Start-Process -Wait`, читает `results.txt` и эмитит `stage-results`. Phase 3 завершила всё, кроме сквозного теста с реальным PlantSim и реальным SimTalk-макросом.

Главная задача этой фазы — **согласовать контракт результатов** (results.txt) с реальным SimTalk-макросом и **отладить запуск конца-в-конец**. Это означает: (1) написать правильный SimTalk-шаблон для `.UserObjects.printed`, (2) верифицировать, что `exitApplication` действительно закрывает PlantSim и PowerShell-обёртка получает exit code 0, (3) убедиться, что results.txt записывается до выхода, (4) Rust читает файл корректно.

**Критическая находка:** В Plant Simulation есть **защита от записи файлов** ("Prohibit access to the computer") — если она активирована в модели, `FileInterface` не сможет писать за пределы папки модели. Это нужно **разблокировать** в настройках модели `.spp`. [CITED: docs.plm.automation.siemens.com/content/plant_sim_help/15/.../prohibit_access_to_the_computer_model_settings.html]

**Primary recommendation:** Основная работа — в SimTalk, а не в Rust. Rust-сторона уже реализована корректно. Фокус на: правильный шаблон SimTalk-метода + проверка security-настройки в PlantSim + тест end-to-end на реальном заводском ПК.
