...
This document describes all configurable settings in Digital Factory (DP_orchestra) вЂ” the runtime settings file, UI state persisted in the browser, the Tauri build configuration, and the release process.
...
The file is created automatically on the first save from the Settings panel (gear icon). If the file is absent on startup, all fields default to empty strings вЂ” the application starts normally in mock mode.
...
| `spp_path` | **Required** (for simulation) | `""` | Absolute path to the `.spp` Plant Simulation model file. Stored in `settings.json` and used in `run_plantsim` automatically вЂ” no file dialog shown at run time. If empty, the pipeline aborts with "РџСѓС‚СЊ Рє .spp РјРѕРґРµР»Рё РЅРµ Р·Р°РґР°РЅ". |
| `sim_method` | **Required** (for simulation) | `""` | SimTalk method name to execute, e.g. `.UserObjects.printed`. Stored in `settings.json`. Also cached in `localStorage` as `lastSimMethod` after each successful run; this cached value is used as a fallback pre-fill when `sim_method` in settings is empty. Only alphanumeric characters, dots, underscores, hyphens, and spaces are accepted вЂ” the backend validates this before launch. |
| `vault_url` | Optional | `""` | Vault PDM server base URL, e.g. `http://192.168.1.10:8080`. An empty string or the literal value `mock` enables mock mode вЂ” the BOM panel is populated with test data instead of contacting a real server. |
...
| `vault_part_number` | Optional | `""` | Default part number used in the BOM query. Pre-fills the `partNumber` query parameter sent to the Vault API. If empty, an empty string is passed вЂ” the Vault server may return an error or a default BOM depending on its configuration. |
...
| `panelLeftPct` | (CSS default) | Left panel width as a percentage (range 20вЂ“80). Set by the resize handle drag interaction; restored on next load. Stored as a decimal string, e.g. `"35.4"`. |
...
**Format:** UTF-8, any number of `key=value` pairs, one per line. All pairs are parsed dynamically вЂ” there is no fixed schema. Empty lines and lines without `=` are ignored.
...
wip=11
lead_time=14.2
bottleneck=РЎС‚Р°РЅС†РёСЏ_5
```

Each parsed `key=value` pair is emitted as a `ResultEntry` and rendered as a dynamic card in the Report tab. The set of keys is not fixed вЂ” add or remove fields in the SimTalk macro and the UI adapts automatically. The file is read only when Plant Simulation exits with a success code.
...
| `identifier` | `com.dp.orchestra` | Unique app identifier |
...
| `installMode` | `currentUser` | Installs to `%LOCALAPPDATA%\Programs\` вЂ” no admin rights needed |
...
1. **Clean build cache** вЂ” Removes `bratsy-tauri/src-tauri/target/release/build/dp-orchestra-*`, `.fingerprint/dp-orchestra-*`, and the previous `dp-orchestra.exe`. This forces the frontend HTML/JS to be re-embedded into the binary.
2. **Build** вЂ” Runs `.\node_modules\.bin\tauri.cmd build` from `bratsy-tauri/`. Exits with code 1 on failure.
3. **Find installer** вЂ” Reads `version` from `tauri.conf.json` and filters `bundle/nsis/` with the glob `*${version}*-setup.exe`. Old installers that do not match the current version are deleted.
4. **Assemble release folder** вЂ” Creates `release/Digital Factory v{version}/` containing `setup.exe` and a `README.txt` with installation instructions.
5. **Pack into ZIP** вЂ” Creates `release/Digital_Factory_v{version}.zip`.
6. **Source archive** вЂ” Runs `git archive HEAD --format=zip` to create `release/Digital_Factory_v{version}_source.zip` (excludes `target/` and `node_modules/`).
